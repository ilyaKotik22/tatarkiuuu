...
class MusicLibrary {
  ...
  loadSongs() {
    return [...Array(20)].map((_, i) => ({
      ...
      coverFile:  `covers/cover${i + 1}.png`   // <─ новая строка
    }));
  }
//   getRandomSong() {
...
async sendRoundMessages(lobby) {
  ...
  /* 2. Отправляем каждому игроку отдельно НОВОЕ сообщение */
  for (const player of lobby.players.values()) {
    if (player.id === lobby.hostId) continue;            // хосту уже отправили
...
// ─── обработка клика по кнопке песни ─────────────────────────────────────
